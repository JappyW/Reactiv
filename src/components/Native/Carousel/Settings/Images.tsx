...
export const CarouselImagesSettings = () => {
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent>
        <div className="flex gap-4 flex-wrap max-h-[200px] overflow-y-auto">
          {images.map((image, index) => (
            <div key={image.id} className="basis-1/3 relative max-w-[200px] max-h-[200px]">
              <img
                src={image.src}
                alt={`${image.src}_removable_image`}
                className="min-w-[200px] min-h-[200px] rounded-md pointer-events-none pt-8"
              />
